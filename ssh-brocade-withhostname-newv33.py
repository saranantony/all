...
def critsit():
    ...
    for cname,i in zip(lotsa_colors,range(len(Lun_name))):
        ...
        plt.xlabel('Time')
        plt.legend(Lun_name,loc="upper center",bbox_to_anchor=(1.30,1.0),bbox_transform=plt.gcf().transFigure,ncol=6)

    plt.savefig('' + my_folder + '\lunreadiocount' + date + '.png',dpi=300,bbox_extra_artists=(lg,),bbox_inches='tight')
    plt.clf()
    plt.close()
    for cname,i in zip(lotsa_colors,range(len(Lun_name))):
        X = []
        ...
        ax = plt.axes()
        ax.xaxis.set_major_locator(dates.HourLocator(interval=2))  # every 4 hours
        ax.xaxis.set_major_formatter(dates.DateFormatter('%H:%M'))  # hours and minutes
        for i in range(len(Y)):
            Y[i] = int(Y[i])
        plt.plot(X, Y,color=lotsa_colors[cname])
        ...
        plt.xlabel('Time')
        lg=plt.legend(Lun_name,loc="upper center",bbox_to_anchor=(1.30,1.0),bbox_transform=plt.gcf().transFigure,ncol=6)

    plt.savefig('' + my_folder + '\lunwriteiocount' + date + '.png',dpi=300,bbox_extra_artists=(lg,),bbox_inches='tight')
    plt.clf()
    plt.close()
    for cname,i in zip(lotsa_colors,range(len(Lun_name))):
        X = []
    ...
    plt.savefig('' + my_folder + '\lunreadbandwidth' + date + '.png',dpi=300,bbox_extra_artists=(lg,),bbox_inches='tight')
    plt.clf()
    plt.close()
    for cname,i in zip(lotsa_colors,range(len(Lun_name))):
        X = []
    ...
    plt.savefig('' + my_folder + '\lunwritebandwidth' + date + '.png',dpi=300,bbox_extra_artists=(lg,),bbox_inches='tight')
    plt.clf()
    plt.close()
    for cname,i in zip(lotsa_colors,range(len(Lun_name))):
        X = []
    ...
    plt.savefig('' + my_folder + '\lunreadlatency' + date + '.png',dpi=300,bbox_extra_artists=(lg,),bbox_inches='tight')
    plt.clf()
    plt.close()
    for cname,i in zip(lotsa_colors,range(len(Lun_name))):
        X = []
    ...
    plt.savefig('' + my_folder + '\lunwritelatency' + date + '.png',dpi=300,bbox_extra_artists=(lg,),bbox_inches='tight')
    plt.clf()
    plt.close()
    for i in range(len(array_name)):
        params1 = (
            ('agentType', 'RAID'),
            ('agentInstanceName', array_name[i]),
            ('fields', 'DATETIME\x1FPORT_NAME\x1FMAX_XFER_RATE\x1FREAD_TOTAL_RESPONSE\x1FWRITE_TOTAL_RESPONSE'),
            ('pfmHostName', array_instancename[i]),
            ('startTime', startTime),
            ('endTime', endTime),
            ('PORT_NAME', portname),
        )
    response1 = requests.get('http://%s:24221/TuningAgent/v1/objects/PI_PTS' % array_instanceip[i], params=params1, verify=False)
    ...
    for cname,i in zip(lotsa_colors,range(len(portname))):
        # plt.plot(df.loc[df['LDEV_NUMBER'] =='%s'%Lun_name[i]]['RECORD_TIME'], df.loc[df['LDEV_NUMBER'] == '%s'%Lun_name[i]]['READ_IO_COUNT'])
        X = []
        Y = []
        X = list(df3.loc[df3['PORT_NAME'] == '%s' % portname[i]]['DATETIME'])
        Y = list(df3.loc[df3['PORT_NAME'] == '%s' % portname[i]]['MAX_XFER_RATE'])
        ax = plt.axes()
        ax.xaxis.set_major_locator(dates.HourLocator(interval=2))  # every 4 hours
        ax.xaxis.set_major_formatter(dates.DateFormatter('%H:%M'))  # hours and minutes
        ...
        plt.xlabel('Time')
        lg=plt.legend(portname,loc="upper center",bbox_to_anchor=(1.30,1.0),bbox_transform=plt.gcf().transFigure,ncol=3)
    plt.savefig('' + my_folder + '\port_throughput' + date + '.png',dpi=300,bbox_extra_artists=(lg,),bbox_inches='tight')
    plt.clf()
    plt.close()
    ...
    for cname,i in zip(lotsa_colors,range(len(portname))):
        ...
        plt.xlabel('Time')
        lg=plt.legend(portname,loc="upper center",bbox_to_anchor=(1.30,1.0),bbox_transform=plt.gcf().transFigure,ncol=3)
    plt.savefig('' + my_folder + '\port_READ_RESPONSE' + date + '.png',dpi=300,bbox_extra_artists=(lg,),bbox_inches='tight')
    plt.clf()
    plt.close()
    for cname,i in zip(lotsa_colors,range(len(portname))):
        X = []
        ...
        plt.xlabel('Time')
        lg=plt.legend(portname,loc="upper center",bbox_to_anchor=(1.30,1.0),bbox_transform=plt.gcf().transFigure,ncol=3)
    plt.savefig('' + my_folder + '\port_WRITE_RESPONSE' + date + '.png',dpi=300,bbox_extra_artists=(lg,),bbox_inches='tight')
    plt.clf()
    plt.close()
    for i in range(len(array_name)):
        params2 = (
    ...
    response2 = requests.get('http://%s:24221/TuningAgent/v1/objects/PI_PRCS' % array_instanceip[i], params=params2, verify=False)
    ...
    for cname,i in zip(lotsa_colors,range(len(adaptorid))):
        X = []
        Y = []
        ...
        plt.xlabel('Time')
        lg=plt.legend(adaptorid,loc="upper center",bbox_to_anchor=(1.30,1.0),bbox_transform=plt.gcf().transFigure,ncol=3)
    plt.savefig('' + my_folder + '\processor' + date + '.png',dpi=300,bbox_extra_artists=(lg,),bbox_inches='tight')
    plt.clf()
    plt.close()
    for i in range(len(array_name)):
        params3 = (
    ...
    response3 = requests.get('http://%s:24221/TuningAgent/v1/objects/PI_CLPS' % array_instanceip[i], params=params3, verify=False)
    ...
    for cname,i in zip(lotsa_colors,range(len(clpr))):
        X = []
        Y = []
        # plt.plot(df.loc[df['LDEV_NUMBER'] =='%s'%Lun_name[i]]['RECORD_TIME'], df.loc[df['LDEV_NUMBER'] == '%s'%Lun_name[i]]['READ_IO_COUNT'])
        ...
        plt.ylabel('CWP')
        plt.xlabel('Time')
        lg=plt.legend(clpr,loc="upper center",bbox_to_anchor=(1.30,1.0),bbox_transform=plt.gcf().transFigure,ncol=3)

    plt.savefig('' + my_folder + '\cwp' + date + '.png',dpi=300,bbox_extra_artists=(lg,),bbox_inches='tight')
    plt.clf()
    plt.close()
if __name__ == '__main__': critsit()